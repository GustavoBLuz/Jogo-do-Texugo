...
#include "corpanda.h" /* our image file */
#include "corchocolate.h" /* our image file */
...
#define _AltuChocolate_ 14
#define _LargChocolate_ 7
...
bool VerificaColisao(int PosXObjeto, 
                     int PosYObjeto, 
					 int PosXAlvo, 
					 int PosYAlvo,
					 int _AltuAlvo_,
					 ...
					 int _LargObjeto_)
{
	...
	/*
	** Rotina para detecção da colisão.
	*/
	int x, y, x2, y2, c, l, c2, l2;
	bool Colidiu = false;
	for(x=0; x<_AltuAlvo_; x++)
	{
		for(y=0; y<_LargAlvo_; y++)
		{
			x2 = x + PosXAlvo;
			y2 = y + PosYAlvo;
			
			for(l=0; l<_AltuObjeto_; l++)
			{
...
int offsetx3, offsety3;
int offsetx4, offsety4;
    int Controle[10][8];
  ...
  Controle[2][1] = offsetx4; // PosX
  Controle[2][2] = offsety4; // PosY
  ...
  // Linha 6: Cachorro
  Controle[6][0] =  6; // CHOCOLATE
  Controle[6][1] = 60; // PosX
  Controle[6][2] = 30; // PosY
  Controle[6][3] = -1; // Sentido (CHOCOLATE controlado pelo teclado)
  ...
  // Linha 7: Panda
  Controle[7][0] =  7; // CHOCOLATE
  Controle[7][1] = 80; // PosX
  Controle[7][2] = 20; // PosY
  Controle[7][3] = -1; // Sentido (CHOCOLATE controlado pelo teclado)
  ...
  // Linha 8: Vendedor
  Controle[8][0] =  8; // CHOCOLATE
  Controle[8][1] = 70; // PosX
  Controle[8][2] = 10; // PosY
  Controle[8][3] = -1; // Sentido (CHOCOLATE controlado pelo teclado)
